...
echo "Running testes on ${QA_ENVIRONMENT} using Browser Driver ${WEB_DRIVER}"
docker run --cpus 4 --memory 3g -e ENVIRONMENT="${QA_ENVIRONMENT}" -e DRIVER="${WEB_DRIVER}" -v $PWD:/app --rm $CONTAINER_REGISTRY/$REPOSITORY/$IMAGE_NAME:latest
