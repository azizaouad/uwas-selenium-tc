...
public class archive_event {
    ...
    public void Choose_archive() {
        try {
            ...
            // List<WebElement> eventLocation =
            // this.driver.getWebDriver().findElements(By.id("even-location"));
    ...
    public void the_event_is_archived() throws InterruptedException {
        ...
        for (int i = 0; i < eventNames.size(); i++) {
            ...
            boolean date = date_string.equals(localDate.toString());
            // boolean url =
            // CurrentUrl.equals("https://recette.uwas.fr/photographer/events/archive");
            if ((name) && (date)) {
                obtainedresult = 1;
                System.out.println((randomName + "  is archived"));
                break;
            }
        }
    ...
    public void Choose_restore_for_the_event() {
        try {
            ...
            List<WebElement> eventNames = this.driver.getWebDriver().findElements(By.id("event-name"));
            // List<WebElement> eventLocation =
            // this.driver.getWebDriver().findElements(By.id("event-location"));
            List<WebElement> eventDate = this.driver.getWebDriver().findElements(By.id("event-date"));
            LocalDate localDate = LocalDate.now();
            if (eventNames.size() == 0) {
                ...
                for (int i = 0; i < eventNames.size(); i++) {
                    ...
                    if ((name) && (date)) {
                        ...
                        restore.click();
                        System.out.println(randomName + "  is restored");
                        break;
                    }
                }
    ...
    public void the_event_is_restored() {
        try {
            // this.driver.getWebDriver().findElement(By.xpath("/html/body/div/div/main/div/div/main/div[1]/button")).click();
            Thread.sleep(3000);
            // boolean found = false;
            int obtainedresult = 0;
            int expectedresult = 1;
