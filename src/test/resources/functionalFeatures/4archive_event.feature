Feature: test archive and restore event functionality
  ...
  Scenario: photographer can archive any event
    Given photographer should login with his credentials email as "b@gmail.com" and password as "Admin123!" and create an event title as "testing"
    When photographer click on the three buttouns
    And Choose archive for the event name as "testing"
    Then the event as "testing" is archived
  ...
  Scenario: photographer can restore event
    ...
    And Choose restore for the event "testing"
    Then the title of event as "testing" is restored
