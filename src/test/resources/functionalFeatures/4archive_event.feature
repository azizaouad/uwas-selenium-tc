Feature: test archive and restore event functionality
  ...
  Scenario: photographer can archive any event
    Given photographer should login with his credentials email as "b@gmail.com" and password as "Admin123!" and create an event title as "testarchivevent"
    When photographer click on the three buttouns
    And Choose archive for the event name as "testarchiveeevent"
    Then the event as "testarchiveeevent" is archived
  ...
  Scenario: photographer can restore event
    ...
    And Choose restore for the event "testarchiveeevent"
    Then the title of event as "testarchiveeevent" is restored
